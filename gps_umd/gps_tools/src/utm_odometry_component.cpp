...
namespace gps_tools
{
  ...
  public:
    explicit UtmOdometryComponent(const rclcpp::NodeOptions& options) :
      ...
      append_zone_(false)
    {
      odom_pub_ = create_publisher<nav_msgs::msg::Odometry>("gps_odom", 10);
      ...
      auto callback = [this](const typename sensor_msgs::msg::NavSatFix::SharedPtr fix) -> void
      {
        ...
        if (odom_pub_) {
          ...
          RCLCPP_INFO(this->get_logger(), "under publish");


        }
      };
      fix_sub_ = create_subscription<sensor_msgs::msg::NavSatFix>("/ublox_rover/fix", 10, callback);
    }
